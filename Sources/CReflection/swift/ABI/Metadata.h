...
#ifndef OPENCOMBINE_SWIFT_ABI_METADATA_H
#define OPENCOMBINE_SWIFT_ABI_METADATA_H
...
struct HeapObject {}; // FIXME: This shoul be forward-declared here and defined somewhere else!!!!!!!!!!!!!!!
class WeakReference;
...
using HeapObjectDestroyer =
  OPENCOMBINE_SWIFT_CALLING_CONVENTION void(OPENCOMBINE_SWIFT_CONTEXT HeapObject *);
...
public:
  ...
  void setAsTypeMetadata() {
    ...
    auto otherSwiftBit = (3ULL - OPENCOMBINE_SWIFT_CLASS_IS_SWIFT_MASK);
...
using AssociatedWitnessTableAccessFunction =
  OPENCOMBINE_SWIFT_CALLING_CONVENTION WitnessTable *(const Metadata *associatedType,
                                                      const Metadata *self,
...
public:
  ...
  span<ProtocolDescriptorRef> getProtocols() const {
...
struct TargetResilientWitnessTable final
  ...
  span<TargetResilientWitness<Runtime>>
  getWitnesses() const {
...
public:
  ...
  /// satisfied
  span<GenericRequirementDescriptor>
...
TargetContextDescriptor<Runtime>::getModuleContext() const {
  ...
  for (auto cur = this; true; cur = cur->Parent.get()) {
    if (auto module = llvm::dyn_cast<TargetModuleContextDescriptor<Runtime>>(cur))
      return module;
  }
}
...
public:
  ...
  span<uint16_t> getGenericParameterCounts() const {
    return span<uint16_t>(this->template getTrailingObjects<uint16_t>(),
                              Flags.getNumGenericParameterLevels());
  }

  /// Retrieve the generic parameters descriptors.
  span<GenericParamDescriptor> getGenericParameters() const {
    return span<GenericParamDescriptor>(
             this->template getTrailingObjects<GenericParamDescriptor>(),
             getGenericParameterCounts().back());
  }

  /// Retrieve the generic requirements.
  span<GenericRequirementDescriptor> getGenericRequirements() const {
    return span<GenericRequirementDescriptor>(
...
public:
  ...
  span<GenericParamDescriptor> getGenericParams() const {
    if (!asSelf()->isGeneric())
      return {};
...
public:
  ...
  string_view getMangledExtendedContext() const {
...
public:
  ...
  /// this protocol.
  span<TargetGenericRequirementDescriptor<Runtime>>
  ...
  span<TargetProtocolRequirement<Runtime>>
...
public:
  ...
  string_view getUnderlyingTypeArgument(unsigned i) const {
    assert(i < getNumUnderlyingTypeArguments());
...
public:
  ...
  MetadataResponse operator()(MetadataRequest request) const {
    using Fn0 = OPENCOMBINE_SWIFT_CALLING_CONVENTION MetadataResponse(MetadataRequest request);
  ...
  MetadataResponse operator()(MetadataRequest request,
    ...
    using Fn1 = OPENCOMBINE_SWIFT_CALLING_CONVENTION MetadataResponse(MetadataRequest request,
                                                 const void *arg0);
  ...
  MetadataResponse operator()(MetadataRequest request,
    ...
    using Fn2 = OPENCOMBINE_SWIFT_CALLING_CONVENTION MetadataResponse(MetadataRequest request,
                                                                      const void *arg0,
                                                                      const void *arg1);
  ...
  MetadataResponse operator()(MetadataRequest request,
    ...
    using Fn3 = OPENCOMBINE_SWIFT_CALLING_CONVENTION MetadataResponse(MetadataRequest request,
                                                                      const void *arg0,
                                                                      const void *arg1,
                                                                      const void *arg2);
...
private:
  ...
  MetadataResponse applyMany(MetadataRequest request,
    ...
    using FnN = OPENCOMBINE_SWIFT_CALLING_CONVENTION MetadataResponse(MetadataRequest request,
...
struct TargetSingletonMetadataInitialization {
  ...
  bool hasResilientClassPattern(
    ...
    auto *classDescription =
      llvm::dyn_cast<TargetClassDescriptor<Runtime>>(description);
...
public:
  ...
  span<MethodDescriptor> getMethodDescriptors() const {
    if (!hasVTable())
      return {};
  ...
  span<MethodOverrideDescriptor> getMethodOverrideDescriptors() const {
    if (!hasOverrideTable())
      return {};
...
int32_t TargetTypeContextDescriptor<Runtime>::getGenericArgumentOffset() const {
  ...
  case ContextDescriptorKind::Struct:
    return llvm::cast<TargetStructDescriptor<Runtime>>(this)
        ->getGenericArgumentOffset();
  default:
    opencombine_swift_runtime_unreachable("Not a type context descriptor.");
  }
}
...
TargetTypeContextDescriptor<Runtime>::getSingletonMetadataInitialization() const {
  ...
  case ContextDescriptorKind::Class:
    return llvm::cast<TargetClassDescriptor<Runtime>>(this)
        ->getSingletonMetadataInitialization();
  default:
    opencombine_swift_runtime_unreachable("Not a enum, struct or class type descriptor.");
  }
}
...
class DynamicReplacementScope
  ...
  span<const DynamicReplacementDescriptor> getReplacementDescriptors() const {
...
#endif // OPENCOMBINE_SWIFT_ABI_METADATA_H
