...
@_spi(TokamakCore) public final class ViewNode<Renderer: GraphRenderer>: CustomDebugStringConvertible {
  ...
  var children: [ViewNode<Renderer>]
  unowned var parent: ViewNode<Renderer>?
...
@_spi(TokamakCore) public extension ViewNode {
  func traverse<Result>(_ work: (ViewNode<Renderer>) -> Result?) -> Result? {
    var stack = children
    while true {
  ...
  func findView(id: ViewNode<Renderer>.Identity) -> ViewNode<Renderer>? {
    traverse { node in
      node.id == id ? node : nil
    }
  }
...
public final class Reconciler<Renderer: GraphRenderer> {
  ...
  enum Mutation {
    case insert(ViewNode<Renderer>, parent: ViewNode<Renderer>, sibling: ViewNode<Renderer>?)
    case remove(ViewNode<Renderer>, parent: ViewNode<Renderer>)
    case replace(
      parent: ViewNode<Renderer>,
      previous: ViewNode<Renderer>,
      current: ViewNode<Renderer>
    )
    case update(ViewNode<Renderer>, newElement: Renderer.ElementType)
  }

  final class InitialTreeBuilderVisitor: ViewVisitor {
    var root: ViewNode<Renderer>
    ...
    func visit<V>(_ view: V) where V: View {
      ...
      var accessors: [(ViewNode<Renderer>, (TreeReducer.Visitor) -> ())] =
        [(root, { $0.visit(view) })]
      while true {
  ...
  final class ReconcilerVisitor: ViewVisitor {
    var current: ViewNode<Renderer>
    var mutations = [Mutation]()

    init(current: ViewNode<Renderer>) {
      self.current = current
    }

    func visit<V>(_ view: V) where V: View {
      var nodeStack = [current]
      var accessorStack: [(ViewNode<Renderer>, (TreeReducer.Visitor) -> ())] =
      ...
      while true {
        ...
        var lastSibling: ViewNode<Renderer>?
        for (index, child) in reducer.result
          .enumerated()
        { // TODO: Reconcile with nextNode.children
  ...
  func reconcile(from current: ViewNode<Renderer>) {
    ...
    for mutation in visitor.mutations {
      ...
      case let .insert(viewNode, parent, sibling):
        ...
        guard let element = viewNode.element,
              let parentElement = parent.element
        else { continue }
        renderableMutations
          .append(.insert(element: element, parent: parentElement, sibling: sibling?.element))
